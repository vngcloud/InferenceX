...
def recipe(row):
    ...
    if row.get('diagnostic'):
        ...
        if row['ep'] == 8:
            ...
            # explicit normal/low_latency modes. Use normal for the diagnostic:
            # it records select_experts output before dispatch and avoids the
            # large fixed buffers required by forcing low_latency for prefill.

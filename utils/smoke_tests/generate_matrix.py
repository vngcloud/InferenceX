...
from discover import DEFAULT_DISCOVER_URL, fetch_discover
    ...
    for stack in stacks:
        ...
        if stack_config is None:
            print(
                ...
                f"{DEFAULT_TEST_CASES} (no throughput).",
                file=sys.stderr,
            )
            ...
            throughput = stack_config.get("throughput")
            if "throughput" in test_cases and throughput is None:
                raise SystemExit(
                    f"'{name}' lists 'throughput' in test-cases but has no "
                    "throughput: block in smoke-tests.yaml."
                )

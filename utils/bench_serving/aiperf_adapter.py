...
from server_metrics import load_server_metrics, extract_cache_stats
...
def build_result(
    ...
    ``server_metrics`` is the parsed content of AIPerf's
    ``server_metrics_export.json`` (written when ``--server-metrics-url`` is
    passed). When provided, cache hit rates are extracted and added to the
    result so ``process_result.py`` can pass them through to the final agg JSON.
    """
    result = {
...
def run_search(args: argparse.Namespace) -> dict:
    ...
    server_metrics = load_server_metrics(artifact_dir / "server_metrics_export.json")
    result = build_result(artifact, winner_conc, server_metrics)

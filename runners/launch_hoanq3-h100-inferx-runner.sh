#!/usr/bin/env bash
# Launch script for the hoanq3-h100-inferx-runner H100 runner (plain Docker,
# no Slurm/enroot). Selected by benchmark-tmpl.yml via:
# bash ./runners/launch_${RUNNER_NAME%%_*}.sh
# where RUNNER_NAME (runner.name) is "hoanq3-h100-inferx-runner" → this script.
# Mirrors launch_h100-greennode.sh; the `_h100` token below is only a serving

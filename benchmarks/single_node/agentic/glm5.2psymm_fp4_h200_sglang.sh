...
# GLM-5.2 W4AFP8 prod-replica + NCCL symmetric memory benchmark: identical
# to glm5.2prod_fp4_h200_sglang.sh except --enable-symm-mem is added
# (which force-enables NCCL NVLS + CUMEM internally, see
# sglang/srt/entrypoints/engine.py::_set_envs_and_config). Measures the
# communication-stack uplift on the TP-MoE prod topology. Note symm-mem
# preallocates ~4GB/GPU (SGLANG_SYMM_MEM_PREALLOC_GB_SIZE default), so
# mem-fraction-static may need lowering if OOM appears.

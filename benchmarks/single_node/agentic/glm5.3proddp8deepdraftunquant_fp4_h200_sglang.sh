...
# GLM-5.3 W4AFP8 diagnostic arm: dequantize the baked MTP draft head to bf16
# (--speculative-draft-model-quantization unquant) while keeping the main
# model at w4afp8.  Byte-identical to glm5.3proddp8deep_fp4_h200_sglang.sh
# except for the single added flag.  Purpose: isolate whether the ~15pp
# MTP accept-rate gap (5.3=0.52 vs 5.2=0.67 at CCU24, same w4afp8 scheme)
# is caused by w4afp8 calibration quality of the draft head (B-hypothesis:
# accept rate recovers to ~0.67) or by the draft model weights themselves
# being a weaker predictor (A-hypothesis: accept rate stays ~0.52).
#
# 诊断 arm：将 MTP draft head 去量化为 bf16（--speculative-draft-model-quantization
# unquant），主模型保持 w4afp8。目的：隔离 5.3 MTP 接受率偏低是由于 draft head
# 的 w4afp8 校准质量（B 假设：接受率恢复至 ~0.67），还是 draft 权重本身预测能力
# 较弱（A 假设：接受率不变 ~0.52）。
...
export MODEL_PATH=$(python3 -c "from huggingface_hub import snapshot_download; print(snapshot_download('PhalaCloud/GLM-5.3-W4AFP8'))")

...
# glm5.2dplm_fp4_h200_sglang.sh except EAGLE runs 5 steps / 6 draft tokens
# instead of 3 / 4. Kept as its own file to get its own MODEL_PREFIX, since
# the conc ladder overlaps the DP sweep at 8 and 16 and agg_bmk.json tells
# arms apart by prefix. Driven with dp-attn false, so the DP branch below is
# dead code here: no router, chunked-prefill 8192, --tp-size only, and
# --cuda-graph-max-bs min(2*CONC, 64).

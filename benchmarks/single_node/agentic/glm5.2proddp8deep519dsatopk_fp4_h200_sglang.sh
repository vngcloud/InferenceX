...
# GLM-5.2 W4AFP8 prod-exact DP8 benchmark, spec-DSA-topk A/B on v0.5.19.
# Identical to glm5.2proddp8deep519_fp4_h200_sglang.sh (run 34508949422) with
# ONE server-arg change: --speculative-dsa-topk-backend flashinfer instead of
# the default sgl-kernel. That flag picks the DSA indexer top-k backend for
# the EAGLE mtp draft worker (v0.5.19 choices: sgl-kernel/torch/flashinfer);
# everything else -- image v0.5.19, --dsa-prefill-backend flashmla_sparse_q8,
# HiCache 128GB/rank write_back, DeepEP EP8, prod env parity -- is unchanged.
# Pinned as its own model-prefix (glm5.2proddp8deep519dsatopk) so both arms'
# result series stay separable and later edits cannot redefine this A/B.

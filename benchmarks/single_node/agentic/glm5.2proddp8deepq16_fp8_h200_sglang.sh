...
# A/B variant of glm5.2proddp8deep_fp8: ONLY change is DSA prefill kernel
# flashmla_sparse_q8 -> flashmla_sparse (bf16 sparse prefill, still fp8 KV; per
# sglang PR #30514 the documented bf16 baseline). Isolates whether the native-FP8
# 4 tok/s prefill collapse (64-tok/step graph replay, run 34212882463) is caused
# by the fp8 sparse-prefill kernel. Everything else identical to the fp8 arm.

...
# AgentX-MVP (cc-traces-weka) smoke for Qwen/Qwen3-8B (bf16) on vLLM
...
#
# Qwen3-8B uses standard GQA (uniform KV specs per layer) — no hybrid attention,
# no HMA required. LMCacheConnectorV1 is fully compatible.
#
# ONE deliberate difference from the SGLang path: aiperf runs in an ISOLATED venv.
# vLLM v0.21.0's API server imports anyio/starlette lazily while serving;
# install_agentic_deps upgrades that web stack and triggers
# `_IncludedRouter has no attribute 'path'` then `TaskHandle` import errors.

...
# Spec-depth arm: glm5.2deep_fp4_h200_sglang.sh with EAGLE 5/1/6 instead of
# 3/1/4, and nothing else touched. Read it against glm5.2deep (run
# 32627106733) -- that pair reads speculative depth alone.
# topk stays 1 because it has to: dsa forces page_size 64 and sglang raises
# "speculative_eagle_topk > 1 with page_size > 1 is only supported on
# (flashinfer, fa3, triton); got attention_backend='dsa'", so tree spec and DSA
# are mutually exclusive. num-draft-tokens tracks steps+1 (topk 1 means the
# draft is a chain, so steps 5 emits 5 draft tokens plus the bonus token).
# Everything else -- dp == tp, ep 8 DeepEP, the two DP-attention sync flags,
# --dsa-prefill-backend flashmla_sparse_q8, lpm, hicache ratio from the config,
# mem-fraction-static 0.80, --disable-shared-experts-fusion -- is unchanged.

...
# decoding, 2-way data parallel (tp1 x dp2 = 2 GPUs), UNQUANTISED KV CACHE.
#
# ONE-FLAG A/B against gemma4mc30_fp8block_h200.sh: this file is that recipe
# with --kv-cache-dtype fp8_e4m3 DROPPED (kv-cache-dtype auto -> the model's
# own bfloat16). Everything else -- checkpoint, EAGLE3 draft, dp2, context
# window, scheduler caps, dataset, telemetry -- is byte-identical, so any delta
# is attributable to the KV dtype and the attention backend it unlocks.
#
# WHY. On H200 (SM90) fp8 KV forces the Triton attention kernel and there is no
# way around it. Proven on this box 2026-08-27 by pinning FA3 explicitly:
#   [fa_utils.py:217] FA3 does not support head_size=512 on SM90:
#                     upgrading FlashAttention 3 -> 4
#   AssertionError: FP8 is only supported on SM100 (compute capability 10.x)
#                   for FA4 CuTe.
# FA3 takes fp8 but not head_size 512; FA4 takes head_size 512 but wants
# Blackwell for fp8. FlashInfer is excluded for gemma4 independently, by
# FlashInferBackend.supports_mm_prefix() -> False (multimodal PrefixLM
# bidirectional attention). So under fp8 the candidate set collapses to
# ['TRITON_ATTN']; drop fp8 and it opens to
# ['FLASH_ATTN', 'TRITON_ATTN', 'FLEX_ATTENTION'] and FLASH_ATTN wins.
# Measured at 8k1k conc 8 (runs 32946716838 vs 32946714039): TTFT mean
# 798 vs 1376 ms, TPOT 17.9 vs 22.5 ms, 0.45 vs 0.36 req/s.
#
# THE TRADE, AND THE POINT OF THIS RUN. bf16 KV is ~2x the bytes per token, so
# the pool roughly halves: the fp8 sibling reported ~1,259,381 tok/rank and
# peaked at kv_usage 58.6% at conc 32, i.e. ~738k tokens live. Against a ~630k
# bf16 pool that does not fit, so conc 32 is EXPECTED to preempt and/or shed
# prefix-cache residency. conc 8/16 have room. The c32 point is kept on purpose
# -- a measured preemption rate is the answer to "can gemma4 agentic afford
# bf16 KV", and an assumed one is not. Read kv_usage, the preemption counter
# and vllm:prefix_cache_* per concurrency, not just the latency summary.
#
...
# allocate outside the granted pair no matter what else is on the box.
#
# 6,7 was granted at dispatch (2026-08-27). NOTE the stale claim above: an
# earlier revision of this recipe recorded GPUs 4-7 as the other tenant's, but
# that has since changed hands -- 6,7 are ours now. dp2 uses BOTH cards, one DP
# replica each. Override with GPU_IDS if the pair moves again.
export CUDA_VISIBLE_DEVICES="${GPU_IDS:-6,7}"
...
# The reason this arm exists: prove the attention backend actually flipped.
# Expect FLASH_ATTN out of ['FLASH_ATTN', 'TRITON_ATTN', 'FLEX_ATTENTION'].
# If this still says TRITON_ATTN then the KV dtype did not land and the run is
# not the A/B it claims to be -- check "non-default args" for kv_cache_dtype.
echo "===== attention backend selection ====="
grep -E "attention backend|FlashAttention version|kv_cache_dtype" "$SERVER_LOG" || true
echo "======================================="

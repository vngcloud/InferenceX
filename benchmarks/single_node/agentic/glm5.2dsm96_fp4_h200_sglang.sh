...
# Tuned DeepEP arm: byte-identical to glm5.2deep_fp4_h200_sglang.sh except for
# --deepep-config (96 SMs instead of the 20-SM default) and
# --expert-balancedness-report-mode server_log. It is the paired half of
# glm5.2deep, so the delta between the two prefixes is those two flags and
# nothing else -- mem-fraction stays 0.80, EAGLE stays 3/1/4, dp == tp, ep 8.
# Kept as its own file to carry its own MODEL_PREFIX: the ladder overlaps the
# glm5.2deep sweep at conc 16, 24 and 40, and agg_bmk.json tells arms apart by
# infmax_model_prefix.

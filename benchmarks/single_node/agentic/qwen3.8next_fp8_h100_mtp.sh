...
# Qwen3.8-Flash-Next FP8 on H100 with SGLang NEXTN MTP. Hopper has no NVFP4
# tensor cores, so this arm serves Qwen/Qwen3.8-Flash-Next-FP8 (172.8 GiB).
# The cookbook has no H100 command; this is the H200 arm adjusted: TP8/EP8
# (TP4 leaves ~43 GiB/rank of weights on an 80 GB card, too little for the
# 256k-capped traces) and --mem-fraction-static 0.75.
#
# Required env vars:
...
# requests up to ~1M tokens the server would reject, and 80 GB is the tightest part here.
...
SGLANG_CMD=(
    ...
    --port "$PORT"
    --trust-remote-code
    # Cookbook H200 / FP8 flags adjusted for H100's 80 GB; NVFP4 is unavailable on Hopper.
    --tp-size "$TP"
    --ep-size "$EP_SIZE"
    --dp-size 1

...
# glm5.2ll7_fp4_h200_sglang.sh except prefill context parallelism is on
# (--attn-cp-size 2 --enable-prefill-cp --cp-strategy interleave), i.e. the
# CP twin of the EAGLE 7/8 arm. Kept as its own file to get its own
# MODEL_PREFIX, since the conc ladder is exactly ll7's [4, 8, 12] and
...
#
# NOTE (verified by on-box boot 2026-09-05, v0.5.18): the DSA override in
# sglang's arg_groups/overrides.py forces attn_cp_size = tp/dp (=8 here),
# so the CLI --attn-cp-size 2 below is overridden to 8 and attn_tp becomes 1
# (attention fully replicated, not TP-sharded). Prefill CUDA graphs are
# disabled; decode keeps its graphs. Boot passed with no crash, EAGLE
# accept_len 5.05 on smoke, KV pool 967,424 tokens/rank (vs 1,173,696 on
# ll7). What the run answers: whether the long-prefill TTFT win outweighs
# the replicated-decode cost on a single node.
...
# Prefill context parallelism (the variable under test vs ll7). NOTE: the

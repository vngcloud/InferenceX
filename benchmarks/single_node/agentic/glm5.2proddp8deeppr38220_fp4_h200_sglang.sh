...
# GLM-5.2 W4AFP8 prod-exact DP8 benchmark, sglang PR #38220 image pin of
# glm5.2proddp8deep519. Byte-identical to glm5.2proddp8deep519_fp4_h200_sglang.sh
# (reference run 34508949422, lmsysorg/sglang:v0.5.19, 2026-09-10) except the
# header you are reading: the single change is the server image,
# sglang-pr38220:v0.5.19-09647177 instead of lmsysorg/sglang:v0.5.19, set via
# the config `image:` key. sglang-pr38220:v0.5.19-09647177 = lmsysorg/sglang:
# v0.5.19 plus sglang PR #38220 ([perf] Optimize w4a8 MoE for glm5.2 on H200,
# branch perf/perf-w4a8-moe-glm52, head 09647177d80c, built by the PR author
# 2026-09-11): tuned CUTLASS w4a8 grouped-GEMM dispatch branches for GLM-5.2's
# exact GEMM shapes. Pinned as its own model-prefix (glm5.2proddp8deeppr38220)
# so later edits to the deep519 arm cannot silently redefine this A/B and the
# result series stay separable.
# NOTE: the image tag exists ONLY locally on the h200-greennode boxes (54.5GB,
# no registry push). The launcher pull-fallback guard (docker pull ||
# docker image inspect, added with this arm) lets the local tag run without a
# registry push; registry-backed images are unaffected (pull succeeds, the
# fallback never fires).
# --hicache-size is PER-RANK GB: 8 ranks * 128 = 1024 GB host RAM total, guard
# N <= free_RAM/8 (~182 GiB/rank headroom) holds.
#
# Prod-exact 2-container topology (same as proddp8deep/deep519). Router stays
# the v0.5.18 router patch build sidecar (sglang-router-patched:v0.5.18-slim-
# noargcanon): the router talks plain HTTP to the server (worker-urls +
# /metrics), so it is not version-coupled to the server image. Router args
# mirror the live glm-52-fp8-h200-8x-router deployment.
#

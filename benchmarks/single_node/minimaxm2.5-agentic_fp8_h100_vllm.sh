#!/usr/bin/env bash
#
# Agentic-replay Mode 1 launcher: MiniMaxAI/MiniMax-M2.5 (FP8, MoE 229B) on vLLM,
# driven by a recorded mooncake_trace JSONL through official AIPerf. Sized for
# 4x H200 (TP=4). FP8 is the native checkpoint precision (auto-detected; no
# --dtype). Uses a dedicated model-prefix `minimaxm2.5-agentic` so it does NOT
# shadow the fixed-seq-len script `minimaxm2.5_fp8_h100.sh`. The `_h100` token in
# the filename is only a serving-script naming convention (hardware-agnostic).
# See docs/AGENTIC_MODE1_CCU_SWEEP.md and docs/AIPERF_INTEGRATION.md.
...
# Expert parallel only when EP_SIZE > 1 (official 4-GPU recipe uses plain TP4).
EP_ARGS=()

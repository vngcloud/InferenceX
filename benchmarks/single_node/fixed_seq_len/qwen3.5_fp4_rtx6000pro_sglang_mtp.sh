...
# SGLang holds back total * (1 - mem-fraction-static) as slack, so the 4.05 GiB per-rank MTP draft head
# must be paid for by raising the fraction: 0.65 and 0.75 left the KV/Mamba pools empty on this 96 GiB SKU,
# 0.85 left only 4.56 GiB free per rank (too thin for a 16k prefill chunk), so 0.80.
MEM_FRAC_STATIC="0.80"
CHUNKED_PREFILL_SIZE=$((ISL * 2))
MAX_PREFILL_TOKENS=$((ISL * 2))
# The client never opens more than CONC connections; a larger Mamba state pool strands memory the draft head needs.

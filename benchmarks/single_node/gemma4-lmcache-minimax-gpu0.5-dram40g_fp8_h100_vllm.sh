...
#
# Variant: GPU util 0.5, LMCache CPU DRAM 40 GB.
# Compared to gemma4-lmcache-minimax_fp8_h100_vllm.sh (GPU util 0.65, DRAM 20 GB):
#   GPU_MEMORY_UTILIZATION=0.5  — lower GPU HBM fraction; more headroom for CPU KV budget.
#   LMCACHE_CPU_DRAM_GB=40      — larger CPU KV budget to measure hit-rate at higher retention.
#
...
#
# Reference: benchmarks/single_node/gemma4-lmcache-minimax_fp8_h100_vllm.sh (GPU 0.65, DRAM 20 GB variant).
...
# GPU util 0.5 — lower HBM fraction to leave DRAM headroom for the 40 GB CPU KV budget.
...
LMCACHE_CPU_DRAM_GB="${LMCACHE_CPU_DRAM_GB:-40}"
